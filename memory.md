...
- **L5.1 收口推送（6 笔，drizzle rc.2 字面 pin 已追认）**：core trunk（state/event/seam shim/database 38 迁移含 credential/#sqlite/chimera-v2.db 隔离）+trunk 测试 12/12（parent 复跑一致）+P0 三重实证（迁移无路径触 chimera.db）。双版本并存（core rc.2 vs chimera beta.19）观察项=L5.2+ 若 chimera 直接消费 core Database 类型会撞跨副本类型墙，届时裁决 catalog 收敛。预存 cross-spawn 环境失败入对账表。
- **G-A 收口推送（5 笔）**：liquid/svelte/vue/mybatis 四提取器与快照 N 终态逐字节一致+测试移植（mybatis 18+vue-store+sveltekit 部分+四语冒烟）。**新发现 resolution 层缺口**（fork 无 vue-router.ts/sveltekit-router.ts/sveltekit-link-synthesizer/vuex dispatch 桥/ui-server screens）=后续 N 对齐批素材；reindex 前旧索引 svelte 行号与 mybatis 节点 id 有偏差（sync 自愈）。
- **L5.2+L5.3 已派**（ses 新×2，均 qwen3.8-max low）：L5.2=credential/integration runtime vendor（Lane=packages/core，零接线纪律）；L5.3=F4-P2（swarm 后台化/预算/dispose 矩阵/closeout，fork 引擎超集前提，Lane=agent+tool/task+session/run-state）。无专配看护（载控+收口闸兜底）。
- **L5.2 收口推送（fd7983f8a 等 4 笔）**：credential/integration/connection/integration.ts 四件与上游逐字节一致（sha 对账）+shim 零扩展（integration 不 import location.ts，tag 形式在 layer-node 图）+trunk 测试 4/4（parent 复跑一致，迁移落隔离库实证）。零接线保持（importer=0）。cross-spawn 环境失败已入对账表。
- **F3 遗留批收口（6 commit，推送待 L5.3 破窗修复）**：D 5 全移植/E 1 先移 1 等价 2 降级/F 2 移植 3 等价/J 2 移植 1 等价/K 6 全降级（project-copy）/O 1 移植 2 等价 1 降级/R 5 移植 3 等价/pinned 族全量移植（HEAD 终态，刻意不搬 Recent 组）。cli 182/0 parent 复跑、config 241/0、审计 1 命中=文件路径含 home/ 误报（豁免）。**F3 全战役（含遗留）就此清零**，遗留仅 K 族（决策#2）+split-footer（决策#3）。计划书已回写。推送阻塞=L5.3 在途 swarm.ts 未提交破窗（pre-push 钩拦工作树）。
- **L5.3 F4-P2 收口推送（179379cb3 等 3 笔）**：①swarm 后台化=worker 注册为引擎前台 job（双层豁免 background_concurrent，AGENTS.md 旧"16-worker 吃满池"表述已作废回写）②预算一致性纯测试锁定（quota 作用域未动=已知陷阱维持）③**真实缺口修复：引擎注册表原无 teardown finalizer——实例 dispose 后孤儿 run 继续烧 token，已补 finalizer（finalizer-safe 原语；Effect.ignoreLogged 在 beta.83 不存在，用 ignoreCause({log:true})）**+closeout 五条协议成文于引擎 doc block+dispose 矩阵三腿测试。验证：862/0 首跑（复跑 plugin-agent-regression 负载 flake 单跑 4.6s 绿=预存家族）。pitfall 候选：Effect v4 beta.83 无 Effect.ignoreLogged。
- **G-B 已派（ses_f336c3d49，qwen3.8-max low）**：CG-33 三件套，严格按序=drift 工具先行（基线量化）→ORDER BY→definitionDelta，验收=错边不劣于基线。上游锚 03893b0/02ee151/2cf63fd，快照参照。
- **战役盘面（2026-09-23 中午）**：已收口=M10/F3 全战役/G-A/L5.1/L5.2/L5.3；在途=G-B；余量=G-C（resolution N 对齐：vue-router/sveltekit-router/sveltekit-link-synthesizer/vuex 桥/ui-server screens+svelte +layout route 命名分歧）→CI 批（明令最后）。**L5 主线仅剩 L5.2 消费接线批（connector auth 后段 ~5-8 人日，依赖 catalog/provider/session-runner v2 树=独立批定义待定）**。
- **L5.4a 收口推送（493ff1137 等 7 笔）**：wildcard/policy/provider/model/catalog/session-schema/session-runner-model 与上游字节一致（runner model 唯一 seam=5 行 @opencode-ai/llm→@coding-chimera/llm import 说明符）+Location shim 扩一行 unbound node（仍零 project 闭包）+**4898263dec provider↔integration 映射随 vendor 自然到位**（catalog/provider/runner-model 三落点全在内）。trunk 21/21（parent 复跑）含 Bearer integration-secret 端到端 roundtrip。**新 pitfall 入册：共享 checkout 禁用 git stash 类全仓操作**（L5.4a builder 曾 stash 波及 G-B WIP，pop 无冲突恢复——命大）。G-B 两笔中途件（drift 工具+ORDER BY）随本波推送。
- L5.4b 遗留定义（builder 报告）：plugin/provider/opencode.ts（zen integration 化 163 行）/connector auth 本体（oauth 树+server 消费面）/sdk gen integrationID 一行（归 L6 v2 面）/上游 runner-model 其余测试用例（依赖 session v2 树）。
- **G-B 收口推送（57cffb55d 等 4 笔）**：CG-33 三件套全落地——**错边 28,725(10.5%)→3,125(1.1%)，9.2× 收敛**（验收线达标；残差=aged 索引对称 flip，force rebuild 可清）。drift 工具=scripts/agent-eval/diff-index-drift.mjs（CHIMERA_DATA_DIR 隔离配方）。NO_REBIND 杀开关契约验证（关掉后 5 用例转红=套件真实覆盖）。**发布提示欠账：存量索引建议一次性 force rebuild（入升级文档）**；宿主 .chimera 索引仍是旧语义（自然 sync 逐步迁移或择机 force rebuild）。
- **L5.4b+G-C 已派**：L5.4b ses_f33357f78（connector auth 本体+plugin provider opencode.ts zen integration 化+**首个允许接线批**：integration/credential/catalog 接 fork server 双 parity 端点；禁动 provider.ts 本体）∥ G-C ses_f33350f15（resolution/frameworks N 对齐：vue-router/sveltekit-router/sveltekit-link-synthesizer/vuex 桥+svelte +layout route 命名对齐+drift 前后对比）。
- **G-C 收口推送（3021379bd 等 9 笔）**：导航契约（edgeKind/alsoTargets/createEdges 扇出）+闭包 helper（object-literal/package-deps/expo-router/nextjs 后二者采纳未注册标 FORK STATUS）+vue-router/sveltekit-router 注册+导航合成器（同步适配，fork 无 SYNTH_PASSES 异步表）+vuex dispatch/commit 桥（store 门+namespace 消歧+120 上限）+**svelte +layout 非 route**（索引语义变更已钉测试）+测试 30 用例。drift 对照实验严谨：错边增量（6.6%→9.5%）实证=33 个并发新文件的树漂移非 G-C（对照臂逐项一致）。**新欠账批次**：①resolution 落后件 ~1200 行（express/react-native/react/python/java/csharp/go/nestjs/rust/swift-objc/expo-modules/vue#764）②缺失件（react-router/tanstack-router/astro/goframe+cics/terraform+expo-router/nextjs 激活）③决策项=ui-server Screens 是否入 fork 路线。fork 独有语义保留清单：authoritative 替代 confidence/同步合成器/isTestPath 并存/kernel framework-extract 门。
- **L5.4b 收口+L5 全线完结**：plugin provider opencode.ts（17 文件字节一致+Plain<T> seam+host seam）+connector auth 9 v2 端点双 parity+首个接线层 v2-integration.ts（chimera-v2.db 恒替换+Location pilot 绑 server cwd）+SDK regen。**战役收官全量套件 5632 tests/26 fail=基线 25 确定族+compaction 负载池，零新增**。L5 完成记录已入计划书；L6 清单=Location 多点/PluginHost 全树/Screens 决策/v2 server+client+codegen 整包/oauth page 消费面/cross-spawn 归因。
- **CI 批开工（用户授权含首跑）**：parent 预侦察发现=typecheck.yml/generate.yml 触发分支 dev 遗留（main 从未触发）；review.yml 装未 scoped chimera 包+opencode/gpt-5.5 待核实；test:ci 新包覆盖待审（core/llm/http-recorder 可能漏）；publish.yml 只审不改（Win 签名引用核对）。builder ses 已派（qwen3.8-max low），收口=parent 推送触发首跑+gh 观察。
