...
- **P5-1 已落地（builder=新会话 qwen3.8-max-0902 low，2026-09-18，1 commit `82a38cd73` 已推送）**：resolution 机制移植五件套——①D7 builtins 合并零冲突（新模块 resolution/js-builtins.ts N 骨架 name-matcher+index 双消费；JS_BUILT_INS 并 WeakMap/WeakSet、TS_PRIMITIVE_TYPES 12 项整体采 N、fork 五族+REACT_HOOKS+GO_STDLIB+PASCAL_PREFIXES 双侧字节同；集合差脚本归档 cbench/k-v2-p5-1/setdiff.ts，INVENTORY「待 parent 复核」项以比对表结清）②D8 双层合并（N 前置过滤 #915 import-kind/#1719 sealed/#1714 裸调用/local-binding shadow × fork veto+三层消歧 rank 后校验；N 拒绝=最终 unresolved 禁晋升次名；C static TU/Go 包域/Rust 模块子树+trait-impl 豁免/7 语言 private 文件域全套）③D9 消费端（this.field.m #1496、rust self.m #1861/self.field.m #1585、inner().m #1683 排他、#645/#608 dotted 链、#1573、cpp auto-init、builtin/primitive 接收者否决 #1566/#1840 进 d2；实测 self.* −1,783/this.x.y −588 收敛）④FN_REF 双臂翻牌（wasm flag=true+ReferenceKind=EdgeKind|'function_ref' wire 零变更+matchFunctionRef/resolveThisMemberFnRef+createEdges→references{fnRef:true}+kernel decode 200 映射；.node 未动；fnRef 边 +546；TS 裸标识符位 value-ref 先发赢但边唯一新测试钉死）⑤卫生批（符号级 imports 边 createEdges (source,target) 去重 (line,col) 最低者留——落两臂共享层=parity 中性；dup 399→2；file→import-node 15,644→1,598）。RESOLVER_RANK 仲裁原位不动。**语义 delta=精度方向**：unresolved 266,188→305,966/edges 336,220→302,312（imports +13,065=#915、calls +22,750=拒绝族、fnr +3,620 新发射类、D9 收敛项）。验证：parity 9 语言 exit 0（refMissing/Extra=0，canonRef 含 referenceKind 实证非空转）、resolution-p51 19/19、kernel 电池 58、钉桩 451、typecheck 绿、重索引 A/B stash 往返确定性；快照 ksd-parity-p51-20260918.json。**最大不确定项=calls −21k 召回侧未经 G 臂/召回矩阵复验（runner 依赖全局二进制重建=端点红线，待授权/CI 移交）**。遗留排队六项（#1108 inferLocalReceiverType 全量移植/deferred this-member 超类型二次 pass/#1230 主过滤接入否/erlang arity+arkts 属性步/matchByQualifiedName #1079/#1180/fnRef×value-ref 标记优先序反转否）待 P5-3 bench 数据定取舍。
- **P5-2 已落地（builder=同会话 resume，2026-09-18，1 commit `f59b20c96` 已推送）**：wave-4 开路由+semantics v5——①9 残留语言 parity 先行验证（唯一漂移类=kernel return_type 带 N bare 形 vs fork wasm RAW；按 P4 scala 判例修 7 .rs RAW 镜像（200 UTF-16 cap util::slice_utf16；java normalize_java_type 仅保 Lombok 路径）；lookupCalleeReturnType 接 chainReceiverTypeName 归一）②DEFAULT_ROUTED += go/java/python/rust/c/cpp/php/ruby/csharp/swift=**19/20 kernel 语言路由**（r 暂缓=零语料；selector 配置 only，wire 契约零变更）③EXTRACTION_SEMANTICS_VERSION 4→5（route-change 条款+P5-1 resolution 面双触发；needsReindex 实录=`graph status`→`Semantics: v4 — does not match v5`）④.node 重建 sha256=`ff65c3ce7a56c007d281f8a89371183849b4b67558ebdca2bc3414f8604d5d7f` 双落点（替 189f7f82；7 腿待 CI）⑤主仓全量重索引 84s：129,318/302,325/305,887/67,162/v5/fnRef 543；vs P5-1 账面 Δ=+5/+13/−79/+7 全本批自身语料；切臂零漂移实证=rust 25 文件节点数前后字节同。**事故记录：stale-process 污染**——P5-1 编辑窗口混码增量 sync 写入 158 条 raw function_ref 边（post-flip 发射×pre-P5-1 resolution），全量重索引清除，full/incremental 双路径验证干净，pre 快照存 cbench/k-v2-p5-2/db-pre/。parity 证据：go 4/java 7（含 Lombok fixture）/python 216（58/58 文件）/rust 448（25/25）/csharp 29/php 3/swift 2 漂移修复，ruby 干净，c/cpp 0 diff+23 defer=政策阀（13.4% 在 10-40% band，生产 seam E2E 含 defer fallback 验证）；residual-9×3+routed-9 回归+post-routing 复跑全 exit 0（快照 ksd-parity-p52-*×7）。验证：cargo test 21/21、test/graph 1503=1484/15 环境基线逐名比对零新增、typecheck 绿、**parent 安静树全量 5,333 pass/25 fail=确定基线（环境15+SDK2+httpapi-config8）精确吻合零新增，负载敏感池零触发**。残余风险：java/php/swift/ruby 小 fixture 语料（1-2 文件）、c/cpp 宏重仓 defer 更高。
- **K-v2 战役代码面收口宣告（2026-09-18，parent）**：P0 盘点判定→P1 契约 append→P2 wasm oracle→P3 kernel re-vendor+跟进→P4 parity 重对账→P5-1 resolution 移植→P5-2 wave-4 路由+v5 全部推送（origin/main=`f59b20c96`）；路线 a 裁定全程执行（双实现比对取强者+证据，fork 五机制保全对账零丢失）。**待用户授权批（「大更新」条件已成就）**：P5-3 bench 重验（召回矩阵 matrix-d/f+G 臂 12 格+tb5-v2/tb6-b+tb3/tb4-v2v3 re-baseline——runner 依赖全局二进制）+8 腿 CI 重编+CI 首跑+Win 签名。可选后续：r 语料补全、混码污染存储层守卫（builder 评估低价值缓行）。上游反馈候选池：scala RAW+P5-2 七语言 RAW 镜像（N kernel bare vs N wasm 不发，fork 裁判取 wasm 臂）。
