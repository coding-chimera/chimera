...
/// The fork's SIMPLER comment cleaner (fork tree-sitter-helpers.ts
/// getPrecedingDocstring inline replaces) — the tsjs arm must byte-match it:
///
///   c.replace(/^\/\*\*?|\*\/$/g, '')   // leading /* or /**, trailing */
///    .replace(/^\/\/\s?/gm, '')        // per-line // (+ one optional space)
///    .replace(/^\s*\*\s?/gm, '')       // per-line block continuation star
///    .trim()
///
/// Divergences from clean_comment_markers are deliberate: no `///`/`//!`
/// third-char strip (the fork leaves `/ <text>`), no lua/pascal/brace block
/// forms, no `--`/`#`/`%` line strips, and NO pre-trim (so a trailing-space
/// comment keeps its `*/` — the `$` anchor fails, exactly like the JS).
fn cleaners_tsjs() -> &'static CleanersTsjs {
    static C: OnceLock<CleanersTsjs> = OnceLock::new();
    C.get_or_init(|| CleanersTsjs {
        block_edges: Regex::new(r"^/\*\*?|\*/$").unwrap(),
        slashes: Regex::new(r"\A//\s?").unwrap(),
        star_cont: Regex::new(r"\A\s*\*\s?").unwrap(),
    })
}
...
/// Fork getPrecedingDocstring cleaning for ONE comment (tsjs arm).
pub fn clean_comment_markers_tsjs(comment: &str) -> String {
    let c = cleaners_tsjs();
    let mut s = c.block_edges.replace_all(comment, "").into_owned();
    s = js_multiline_strip(&s, &c.slashes);
    s = js_multiline_strip(&s, &c.star_cont);
    s.trim().to_string()
}

/// Fork getPrecedingDocstring (tree-sitter-helpers.ts) — NO wrapper climb:
/// the fork reads `node.previousNamedSibling` directly, so a comment before
/// `export const x = ...` attaches to NOTHING (the export_statement wrapper
/// owns the comment, and the fork never climbs to it). The tsjs arm must
/// reproduce that byte-for-byte; the #780 wrapper-climb variant above is the
/// upstream shape the other kernel walkers keep.
pub fn preceding_docstring_tsjs(node: Node, src: &str) -> Option<String> {
    ...
    Some(
        comments
            .iter()
            .map(|c| clean_comment_markers_tsjs(c))
            .collect::<Vec<_>>()
            .join("\n")
            .trim()
            .to_string(),
