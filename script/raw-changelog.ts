...
const repo = process.env.GH_REPO ?? "anomalyco/coding-chimera"
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/chimera packages/sdk packages/plugin packages/desktop packages/app sdks/vscode github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/chimera/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/chimera/")) areas.add("core")
