...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/chimera")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with Chimera...`)
  ...
  try {
    await $`chimera run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  Chimera failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with Chimera")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with Chimera...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/chimera`.",
