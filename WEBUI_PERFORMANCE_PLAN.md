...
日期：2026-09-17
状态：已评审（parent 代用户评审通过 2026-09-17；P1/P2 即日开工，P3 门控=W2 合入；开放问题 1-3 授权实施期自裁，内存预算默认 4096MB 起步）
...
2. 驱逐规则：
   ...
   - `maxActiveInstances` 退役：配置键保留解析作兼容，默认禁用（0 = 不限），仅作显式安全阀。
...
1. 内存预算默认值需要实测标定（建议初值 4096MB，暴露配置）。
