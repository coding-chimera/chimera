...
This setting applies across all interfaces: TUI, CLI (`chimera run`), desktop app, and GitHub Action.
...
- `remote` - Remote compaction policy: `"auto"` uses remote compaction when the selected provider and model explicitly support it, `"on"` requests remote compaction but still requires that explicit capability, and `"off"` always compacts locally (default: `"auto"`). Neither `"auto"` nor `"on"` can bypass provider or model capability checks. If a remote attempt fails, Chimera falls back to local compaction.
- `remote_protocol` - Protocol preference: `"auto"`, `"v2"`, or `"legacy"` (default: `"auto"`). Chimera intersects this preference with the provider-authorized protocol order; it never switches to a different provider host to satisfy the preference.
...
:::caution[Remote compaction disclosure]
Remote compaction sends substantial conversation history—including prompts and tool output—to the selected provider. Enable it only for providers and endpoints you trust and have verified.
:::

After successful remote compaction, the session contains opaque state produced by that provider. Chimera then blocks changing the provider or logical model so that state cannot be sent to an incompatible destination. If the same logical model later resolves through a different route, Chimera does not send the opaque state; where safe, it falls back to the full conversation history instead.
