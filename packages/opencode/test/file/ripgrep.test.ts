...
describe("file.ripgrep", () => {
  test("defaults to include hidden", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, ".chimera", "thing.json"), "{}")
      },
    })
    ...
    expect(files.includes(path.join(".chimera", "thing.json"))).toBe(true)
  })
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".chimera", "thing.json"))).toBe(false)
  })
