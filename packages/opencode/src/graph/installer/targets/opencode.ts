...
class OpencodeTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    const alreadyConfigured = !!(config.mcp?.chimera ?? config.mcp?.codegraph);
  ...
  uninstall(loc: Location): WriteResult {
    ...
    if (!fs.existsSync(file)) {
      ...
      if (!config.mcp?.chimera && !config.mcp?.codegraph) {
        files.push({ path: file, action: 'not-found' });
      } else {
        // Drop both the new Chimera key and the legacy CodeGraph key
        // surgically. Leaves siblings + comments untouched.
        let updated = text;
        for (const key of ['chimera', 'codegraph']) {
          if (!parseConfig(updated).mcp?.[key]) continue;
          updated = applyEdits(updated, modify(updated, ['mcp', key], undefined, {
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({
      $schema: 'https://opencode.ai/config.json',
      mcp: { chimera: getOpencodeServerEntry() },
    }, null, 2);
...
function writeMcpEntry(loc: Location): WriteResult['files'][number] {
  ...
  const config = parseConfig(text);
  const before = config.mcp?.chimera ?? config.mcp?.codegraph;
  ...
  const edits = modify(text, ['mcp', 'chimera'], after, {
    formattingOptions: FORMATTING,
  });
