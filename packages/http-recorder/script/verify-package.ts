...
export const verifyPackage = async (archive: string) => {
  ...
  try {
    ...
    await writeFile(
      ...
      `import { HttpRecorder } from "@coding-chimera/http-recorder"
...
`,
    ...
    await run(
      [
        ...
        "--input-type=module",
        "-e",
        'import("@coding-chimera/http-recorder").then((module) => { const root = Object.keys(module).sort(); const namespace = Object.keys(module.HttpRecorder).sort(); if (JSON.stringify(root) !== JSON.stringify(["HttpRecorder"])) throw new Error(`Unexpected root exports: ${root}`); if (JSON.stringify(namespace) !== JSON.stringify(["http", "socket"])) throw new Error(`Unexpected namespace exports: ${namespace}`) })',
      ],
      directory,
    )
