...
**Verify any time** with `codegraph_status` (via MCP) or `chimera status` (CLI). If anything is pending, you'll see a `### Pending sync:` section naming the files and their edit age.

The handful of cases where manual `chimera sync` makes sense: the watcher is disabled (sandboxed environments, or `CODEGRAPH_NO_DAEMON=1`), or you're scripting against the index outside an agent session and want a pre-flight sync at the start of your script.
...
```bash
npx @opencode-ai/chimera
```

The installer will:
- Ask which agent(s) to configure — auto-detects installed ones from: **Claude Code**, **Cursor**, **Codex CLI**, **opencode**, **Hermes Agent**, **Gemini CLI**, **Antigravity IDE**, **Kiro**
- Prompt to install `chimera` on your PATH (so agents can launch the MCP server)
- Ask whether configs apply to all your projects or just this one
- Write each chosen agent's MCP server config (the Chimera usage guide is delivered by the MCP server itself, so no instructions file is added to `CLAUDE.md` / `AGENTS.md` / etc.)
...
```bash
chimera install --yes                              # auto-detect agents, install global
chimera install --target=cursor,claude --yes       # explicit target list
chimera install --target=auto --location=local     # detected agents, project-local
chimera install --print-config codex               # print snippet, no file writes
```
...
Builds the per-project knowledge graph index. A single global `chimera install` works in every project you open — no need to re-run the installer per project.

That's it — your agent will use Chimera tools automatically when a `.codegraph/` directory exists.
...
**Install globally:**
```bash
npm install -g @opencode-ai/chimera
```
...
- If `.codegraph/` doesn't exist yet, offer to run `chimera init -i`.
...
│       calls Chimera tools directly — no Explore sub-agent       │
│                                 │                                 │
└─────────────────────────────────┬─────────────────────────────────┘
                                  │
                                  ▼
...
```bash
chimera                           # Run interactive installer
chimera install                 # Run installer (explicit)
chimera uninstall               # Remove CodeGraph from your agents (inverse of install)
chimera init [path]             # Initialize in a project (--index to also index)
chimera uninit [path]           # Remove CodeGraph from a project (--force to skip prompt)
chimera index [path]            # Full index (--force to re-index, --quiet for less output)
chimera sync [path]             # Incremental update
chimera status [path]           # Show statistics
chimera query <search>          # Search symbols (--kind, --limit, --json)
chimera files [path]            # Show file structure (--format, --filter, --max-depth, --json)
chimera callers <symbol>        # Find what calls a function/method (--limit, --json)
chimera callees <symbol>        # Find what a function/method calls (--limit, --json)
chimera impact <symbol>         # Analyze what code is affected by changing a symbol (--depth, --json)
chimera affected [files...]     # Find test files affected by changes (see below)
chimera serve --mcp             # Start MCP server
```
...
```bash
chimera affected src/utils.ts src/api.ts         # Pass files as arguments
git diff --name-only | chimera affected --stdin   # Pipe from git diff
chimera affected src/auth.ts --filter "e2e/*"     # Custom test file pattern
```
...
```bash
#!/usr/bin/env bash
AFFECTED=$(git diff --name-only HEAD | chimera affected --stdin --quiet)
if [ -n "$AFFECTED" ]; then
...
Chimera can be embedded directly. The npm package re-exports its programmatic
...
```typescript
import CodeGraph from '@opencode-ai/chimera';
// CommonJS works too:
//   const { CodeGraph } = require('@opencode-ai/chimera');
...
- Install from npm (`npm i @opencode-ai/chimera`) so the matching
...
There isn't any — Chimera is zero-config, with **no config file** to write or
...
**"Chimera not initialized"** — Run `chimera init` in your project directory first.

**Indexing is slow** — Check that `node_modules` and other large directories are excluded. Use `--quiet` to reduce output overhead.

**MCP hits `database is locked`** — current builds shouldn't: CodeGraph bundles its own Node runtime and uses Node's built-in `node:sqlite` in WAL mode, where concurrent reads never block on a writer. If you still see it:

- **You're on an old (pre-0.9) install.** Reinstall to get the bundled runtime — `curl -fsSL https://raw.githubusercontent.com/colbymchenry/codegraph/main/install.sh | sh` (macOS/Linux), `irm https://raw.githubusercontent.com/colbymchenry/codegraph/main/install.ps1 | iex` (Windows), or `npm i -g @opencode-ai/chimera@latest`.
- **`chimera status` shows `Journal:` other than `wal`** — WAL couldn't be enabled on this filesystem (common on network shares and WSL2 `/mnt`), so reads can block on writes. Move the project (with its `.codegraph/` folder) onto a local disk.

**MCP server not connecting** — Ensure the project is initialized/indexed, verify the path in your MCP config, and check that `chimera serve --mcp` works from the command line.

**Missing symbols** — The MCP server auto-syncs on save (wait a couple seconds). Run `chimera sync` manually if needed. Check that the file's language is supported and isn't inside a `.gitignore`d or default-excluded directory (e.g. `node_modules`, `dist`).
