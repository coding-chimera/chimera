    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from chimera.json provider config
  ...
  Effect.gen(function* () {
        ...
        try {
          const language = await (async () => {
            ...
            if (s.modelLoaders[canonical.providerID])
              return s.modelLoaders[canonical.providerID](
                sdk,
                canonical.api.id,
                {
                  ...provider.options,
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      if (experimental.model) {
        return {
          ...experimental.model,
          id: ModelID.make(experimental.model.id),
          providerID: ProviderID.make(experimental.model.providerID),
        }
      }
