 ...
 * was then auto-migrated by Antigravity, and re-ran `chimera install`
 * doesn't end up with stale graph entries in two files.
 *
 ...
 * to get Chimera to appear in the Customizations UI. We build the
 ...
 * tools live outside that, so a bare `chimera` command fails to spawn
 * even when `which chimera` resolves in the user's shell. We resolve
 * `chimera` to its absolute path on macOS at install time. (Linux GUI
 ...
 * back to the bare `chimera` name when:
 *
...
function resolveCodegraphCommand(): string {
  if (process.platform !== 'darwin') return 'chimera';
  try {
    const resolved = execSync('command -v chimera || which chimera', {
      encoding: 'utf-8',
...
class AntigravityTarget implements AgentTarget {
  ...
  detect(loc: Location): DetectionResult {
    ...
    const alreadyConfigured = !!(config.mcpServers?.chimera ?? config.mcpServers?.codegraph);
  ...
  printConfig(loc: Location): string {
    ...
    const file = preferredMcpConfigPath();
    const snippet = JSON.stringify({ mcpServers: { chimera: buildAntigravityEntry() } }, null, 2);
    return `# Add to ${file}\n\n${snippet}\n`;
  }
...
function writeMcpEntry(): WriteResult['files'][number] {
  ...
  const before = existing.mcpServers?.chimera;
  const legacyBefore = existing.mcpServers?.codegraph;
  const after = buildAntigravityEntry();

  if (jsonDeepEqual(before, after) && !legacyBefore) {
    return { path: file, action: 'unchanged' };
  }
  const action: 'created' | 'updated' =
    before || legacyBefore ? 'updated' : (fs.existsSync(file) ? 'updated' : 'created');
  if (!existing.mcpServers) existing.mcpServers = {};
  delete existing.mcpServers.codegraph;
  existing.mcpServers.chimera = after;
...
function cleanupLegacyEntry(): WriteResult['files'][number] | null {
  ...
  if (!config.mcpServers?.chimera && !config.mcpServers?.codegraph) return null;
  delete config.mcpServers.chimera;
...
function removeCodegraphFromFile(file: string): WriteResult['files'][number] {
  ...
  const config = readJsonFile(file);
  if (!config.mcpServers?.chimera && !config.mcpServers?.codegraph) return { path: file, action: 'not-found' };
  delete config.mcpServers.chimera;
  delete config.mcpServers.codegraph;
  if (Object.keys(config.mcpServers).length === 0) {
    delete config.mcpServers;
