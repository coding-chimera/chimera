...
class ClaudeCodeTarget implements AgentTarget {
  ...
  uninstall(loc: Location): WriteResult {
    ...
    const mcpPath = mcpJsonPath(loc);
    const config = readJsonFile(mcpPath);
    if (config.mcpServers?.chimera || config.mcpServers?.codegraph) {
    ...
    if (Array.isArray(settings.permissions?.allow)) {
      ...
      settings.permissions.allow = settings.permissions.allow.filter(
        (p: string) => !p.startsWith('mcp__chimera__') && !p.startsWith('mcp__codegraph__'),
      );
  ...
  printConfig(loc: Location): string {
    ...
    const snippet = JSON.stringify({ mcpServers: { chimera: getMcpServerConfig() } }, null, 2);
...
export function writeMcpEntry(loc: Location): WriteResult['files'][number] {
  ...
  const action: 'created' | 'updated' = before || legacyBefore ? 'updated' : (fs.existsSync(file) ? 'updated' : 'created');
