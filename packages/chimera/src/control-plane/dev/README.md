This is a plugin to simulate a remote environment locally. Add this to `.chimera/chimera.jsonc`:

```json
  "plugin": ["../packages/chimera/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate Chimera server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/chimera/script/run-workspace-server
```

With the plugin installed, you can now run Chimera and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
