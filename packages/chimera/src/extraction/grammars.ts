...
function loadTreeSitterRuntime(): TreeSitterRuntime {
  const direct = require('web-tree-sitter');
  const directRuntime = treeSitterRuntime(direct);
  if (directRuntime) return directRuntime;

  const packageRoot = path.dirname(require.resolve('web-tree-sitter/package.json'));

  for (const fallbackPath of [
    'web-tree-sitter/tree-sitter.cjs',
    path.join(packageRoot, 'tree-sitter.cjs'),
