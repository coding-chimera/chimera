/**
 * Project writer lock (fork port of upstream 7440d2c47, #1740/#1744).
 *
 ...
 * lock closes the same-OS gap where two direct-mode `chimera graph serve
 * --mcp` processes (via `CODEGRAPH_NO_DAEMON=1` or proxy→in-process fallback)
 * each start a watcher, contend on `codegraph.lock`, and degrade auto-sync.
 *
 * Deliberately separate from `daemon.pid`: a direct-mode holder must not look
 * like a daemon, and daemon-lock takeover arbitration must not touch the
 * writer slot. `writer.pid` is only about "who owns live auto-sync".
 */
...
export interface WriterLockInfo {
  pid: number;
  /** `direct` | `daemon` | `fallback` | `cleanup` — for actionable error text only. */
  mode: string;
  startedAt: number;
}
 ...
 * re-entrant for the SAME pid (a daemon that already holds the slot keeps it
 * when its engine re-acquires for the watcher).
 */
export function tryAcquireWriterLock(
    ...
    try {
      ...
      try {
        ...
        if ((err as NodeJS.ErrnoException).code === 'EEXIST') {
          ...
          // No hard links on this filesystem — O_EXCL create.
          try {
  ...
  if (result.kind === 'taken') {
    ...
    if (!existing || existing.pid <= 0 || !isProcessAlive(existing.pid)) {
      // Stale — clear (pid-verified, re-read immediately before the unlink)
      // and retry once.
      try {
        const cur = decodeWriterLockInfo(fs.readFileSync(pidPath, 'utf8'));
...
export function writerLockHeldMessage(
  ...
  const who = existing && existing.pid > 0
    ...
    : 'another process';
  return (
    'Chimera graph writer lock held by ' + who + '. ' +
    'Only one live MCP writer may serve a project (auto-sync / index). ' +
    'Stop the other server (end the other MCP session, or kill the shared daemon process), ' +
