...
export class ToolHandler {
  ...
  private async handleStatus(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    lines.push(
      backend === 'bun-sqlite'
        ? `**Backend:** bun:sqlite (Bun built-in) — full WAL + FTS5`
        : `**Backend:** node:sqlite (Node built-in) — full WAL + FTS5`
    );

    // Effective journal mode. 'wal' ⇒ concurrent reads never block on a writer;
    // anything else ⇒ they can ("database is locked"). The active backend
    // supports WAL, so a non-wal mode means the filesystem can't (network/
