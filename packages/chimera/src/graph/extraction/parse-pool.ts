 ...
 * `graph index` uses every core instead of pinning one.
 *
 ...
 * (upstream #1015, the parse-time half of #320). Spreading the parse calls
 * across a pool of workers — each its own tree-sitter WASM heap — restores
 * multi-core throughput. SQLite storage stays on the main thread (it isn't
 * thread-safe), so only the CPU-bound parse step is parallelised; results are
 * stored as they arrive, in whatever order they finish.
 *
 * Design mirrors the upstream query-pool (idle-list dispatch, lazy growth,
 ...
 *     worker is torn down and replaced after `recycleInterval` parses to
 *     reclaim its heap — the same reason the old single worker recycled.
 *   - reject, don't retry: a parse that crashes or times out its worker
 *     REJECTS (with a message the orchestrator's retry pass recognises)
 *     rather than being silently requeued — the orchestrator owns the smarter
 *     two-stage retry (fresh worker, then comment-stripped) on a clean WASM
 *     heap.
 *   - a size-1 pool reproduces the old single-worker path exactly, which is
 *     the conservative rollback: set `CODEGRAPH_PARSE_WORKERS=1`.
 *
 * Memory: peak scales with pool size (≈ size × a worker's pre-recycle heap),
 * so the default is capped and the env var lets constrained machines dial it
 * down.
 */
 ...
 * queue / growth / recycle / crash-recovery logic without spawning threads or
 * a built `dist/`.
 */
...
/** A single file to parse. `language` is optional — when omitted the worker
 *  resolves it itself (this engine's parse-worker falls back to detection). */
 ...
 * upstream #1231) the timers phase runs before the poll phase, so the expired
 * timer fires BEFORE an already-delivered `parse-result` is processed.
...
/**
 * Max workers cold-starting at once. A worker's cold start is heavy (module
 * load + grammar WASM compile); starting the whole pool simultaneously
 * thrashes CPU. Warming a couple at a time keeps each start fast while the
 * pool still reaches full size within a few parses of a large run.
 */
 ...
 * respawning forever. Set high: normal per-file WASM crashes are cleared by
 * the orchestrator's retry pass and shouldn't trip this on a merely-crashy
 * repo.
 */
...
export class ParseWorkerPool {
  ...
  // Spawned but not yet 'grammars-loaded'. Growth counts these so a single
   ...
   * REJECTS if the parse times out or its worker crashes — the caller records
   * the error and (for worker-exit/OOM/timeout rejections) re-attempts in its
   * retry pass.
   */
  ...
  private onMessage(w: ParsePoolWorker, m: ParseWorkerMessage): void {
    ...
    if (m.type === 'parse-result') {
      ...
      if (job.timerExpired) {
        ...
        // and dropped files (upstream #1231).
  ...
  /** Tear down a worker that has hit its recycle threshold and replace it. Not
   *  a crash, so it doesn't count against the budget. */
  ...
  private dispatch(w: ParsePoolWorker, job: ParseJob): void {
    ...
    // original single-worker formula so pathological-file behaviour is
    // unchanged).
   ...
   * synchronous main-thread stretch the timers phase runs before the poll
   * phase, so an already-delivered `parse-result` is still queued behind this
   * callback. Mark the job late (onMessage accepts a result that shows up)
  ...
  private drain(): void {
    ...
    // already on their way up — throttled so we never cold-start the whole
    // pool at once.
    while (
