...
import type { Node as SyntaxNode, Tree } from '../web-tree-sitter-types';
...
// ---------------------------------------------------------------------------
// Fork feature patch set (K-v2 P2 replay onto the N engine base). These are
// the fork-only mechanisms with no upstream counterpart (or an upstream
// counterpart deliberately merged per the P0 rulings D1/D2/D3/D5):
// statement emission, value-position refs, params/RAW returnType, and their
// supporting tables. Upstream snapshot: ba3c21e50d.
// ---------------------------------------------------------------------------
...
/**
 * K-v2 P2 (D5): the Scala capitalized-primitives block of BUILTIN_TYPES is
 * language-scoped — it must NOT suppress type refs in the TS/JS family.
 * `ServerConnection.Any` (nested_type_identifier leaf `Any`) is a real type
 * dependency in TS; blanket suppression cost the main repo −10 edges
 * (kernel-parity tsjs-p2 evidence). Scala/PHP/Java etc. keep the upstream
 * suppression — their grammars genuinely spell primitives capitalized, and
 * N's TYPE_ANNOTATION_LANGUAGES includes scala/php, so deleting the block
 * outright would leak `Int`/`Any` as unresolved-ref noise there.
 */
const BUILTIN_TYPES_CAPITALIZED: ReadonlySet<string> = new Set([
  'Int', 'Long', 'Short', 'Byte', 'Float', 'Double', 'Boolean', 'Char', 'Unit',
  'String', 'Any', 'AnyRef', 'AnyVal', 'Nothing', 'Null',
]);
const BUILTIN_CAPITALIZED_EXEMPT_LANGUAGES: ReadonlySet<string> = new Set([
...
export class TreeSitterExtractor {
  ...
  // K-v2 P2 (D1 merge): the tsjs family is served by the fork's value-position
  // trunk (valueReferenceTypes hook → extractValueReference, cross-file
  // 'references' refs). typescript/javascript/tsx — and arkts, whose config
  // spreads the typescript one and therefore inherits valueReferenceTypes —
  // are REMOVED from this set: keeping them fires the same identifier twice
  // (trunk ref + same-file valueRef edge). The remaining languages keep the
  // upstream capture/flush mechanism (shadow-prune included) untouched; note
  // the fork kernel arm only mirrors the trunk, so kotlin/scala/dart valueRef
  // edges are wasm-arm-only until P3 (P4 whitelist, per P0 finding ⑥).
  private static readonly VALUE_REF_LANGS = new Set<string>(['go', 'python', 'rust', 'ruby', 'c', 'java', 'csharp', 'php', 'scala', 'kotlin', 'swift', 'dart', 'pascal']);
  ...
  // Fork value-position trunk (K-v2 P2 replay): per-file dedup of emitted
  // value refs, keyed `${fromNodeId}\u0000${name}`.
  private valueReferenceKeys = new Set<string>();
  // K-v2 P2 (D3): ids of TS interface member nodes minted during
  // extractInterface's walk — stable-reordered to the nodes-array tail at the
  ...
  extract(): ExtractionResult {
    ...
    try {
      ...
      // K-v2 P2 (D3): the fork's contract-member ordering guarantee, replayed
      // as a post-walk STABLE REORDER over N's #1638 inline real nodes. A
      // contract member name (`interface Store { reset() }`) routinely matches
      // a concrete implementation later in the same file (`reset: () => ...`),
      // and first-match-by-name consumers must keep seeing the executable
      // declaration first: db/queries getNodesByName has no ORDER BY (rowid =
      // this array's order) and name-matcher resolveMethodOnType takes
      // matches[0] (a84c65722 fixed 426 receiver-typed resolutions this way).
      // Node ids carry line numbers and edges reference ids, so array position
      // is order-neutral for the graph itself. Scope equals the OLD deferred-
      // flush scope exactly: TS contract languages, interface members only
      // (type-alias members were and stay inline). P4 keeps the harness
      // node:order-mismatch expectation (kernel arm emits source order).
      if (this.interfaceMemberNodeIds.size > 0) {
        const members: Node[] = [];
        const rest: Node[] = [];
        for (const n of this.nodes) {
          if (this.interfaceMemberNodeIds.has(n.id)) members.push(n);
          else rest.push(n);
        }
        this.nodes = rest.concat(members);
      }
  ...
  private visitNode(node: SyntaxNode): void {
    ...
    // Value-position identifiers (fork trunk, JS/TS family). The generalized
    // module-level walk collects them here; declaration handlers below walk
    // their own bodies. Kept independent of skipChildren so declarations
    // still short-circuit.
  ...
  private extractFunction(node: SyntaxNode, nameOverride?: string): void {
    ...
    // K-v2 P2 replay: the fork's RAW return-type wire field (returnTypeText —
    // verbatim annotation, resolver classifies the shape; feeds the MMS/MCC
    // signature-diff audit) takes precedence over the normalized getReturnType
    // hook value; the hook (N #645, C/C++ bare-name form) fills in only where
    // the declaration carries no returnField annotation.
    const returnTypeAnnotation = this.returnTypeNode(node);
    const returnType = this.returnTypeText(returnTypeAnnotation)
      ?? this.extractor.getReturnType?.(node, this.source);
    ...
    this.extractTypeAnnotations(node, funcNode.id, returnTypeAnnotation);
  ...
  private extractMethod(node: SyntaxNode): void {
    ...
    // K-v2 P2 replay: fork RAW returnType takes precedence over the N #645
    // normalized getReturnType hook (see extractFunction).
    const returnTypeAnnotation = this.returnTypeNode(node);
    const returnType = this.returnTypeText(returnTypeAnnotation)
      ?? this.extractor.getReturnType?.(node, this.source);
    ...
    // Extract type annotations (parameter types and return type)
    this.extractTypeAnnotations(node, methodNode.id, returnTypeAnnotation);
  ...
  private extractInterface(node: SyntaxNode): void {
    ...
    // K-v2 P2 (D3): #1638 mints member nodes INLINE (source order); record the
    // nodes created during this walk so extract() can stable-reorder them to
    // the array tail — the fork's contract-member ordering guarantee (see
    // extract()). Scope equals the old pendingInterfaceMembers deferred-flush
    // scope exactly: TS contract languages, this interface-body walk only.
    const trackMemberOrder =
      kind === 'interface' && (this.language === 'typescript' || this.language === 'tsx');
    const membersStart = trackMemberOrder ? this.nodes.length : -1;
    ...
    if (membersStart >= 0) {
      for (let i = membersStart; i < this.nodes.length; i++) {
        this.interfaceMemberNodeIds.add(this.nodes[i].id);
      }
    }
    this.nodeStack.pop();
  }
  ...
  private extractProperty(node: SyntaxNode): Node | null {
    ...
    // K-v2 P2 replay (fork contract-member promotion over N's #1638 interface
    // path): a function-typed property_signature (`cb: () => void`) is
    // method-shaped on the contract — N's extractTsTypeAliasMembers keeps this
    // promotion for type aliases (memberKind rule), and the fork resolver
    // depends on it for interfaces too: resolveMethodOnType binds `obj.cb()`
    // ONLY to `kind === 'method'` nodes under the container (Strategy
    // 0.5/0.5b), so demoting it to 'property' would silently drop the call
    // binding for every function-typed interface member. TS contract languages
    // only; the signature keeps the N shape.
    const promotedToMethod =
      (this.language === 'typescript' || this.language === 'tsx') &&
      node.type === 'property_signature' &&
      this.isTsFunctionTypedProperty(node);
    const propNode = this.createNode(promotedToMethod ? 'method' : 'property', name, node, {
      docstring,
      signature,
      visibility,
      isStatic,
  ...
  // -------------------------------------------------------------------------
  // Fork feature replay (K-v2 P2): statement emission (D2), value-position
  // references (D1 trunk), RAW returnType + params wire fields, and the D5
  // builtin-type scoping helper. Verbatim fork implementations on the N base.
  // -------------------------------------------------------------------------
   ...
   * tag/attribute names, destructuring bindings, and locally re-bound names
   * (shadow-prune, D1 merge) are skipped.
   */
  ...
  /**
   * BUILTIN_TYPES membership with the K-v2 P2 D5 replay applied: the Scala
   * capitalized-primitives block is LANGUAGE-SCOPED — it does not suppress
   * type refs in the TS/JS family, where `Any`/`String`/… appear as real
   * project type leaves (`ServerConnection.Any`, a nested_type_identifier
   * whose leaf is `Any`; blanket suppression cost the main repo −10 edges,
   * kernel-parity tsjs-p2 evidence). Scala/PHP/Java etc. keep the upstream
   * suppression — N's TYPE_ANNOTATION_LANGUAGES includes scala/php, so
   * deleting the block outright would leak `Int`/`Any` as unresolved-ref
   * noise there.
   */
  private isBuiltinType(name: string): boolean {
    if (!this.BUILTIN_TYPES.has(name)) return false;
    return !(
      BUILTIN_TYPES_CAPITALIZED.has(name) &&
      BUILTIN_CAPITALIZED_EXEMPT_LANGUAGES.has(this.language)
    );
  }
  ...
  private visitFunctionBody(body: SyntaxNode, _functionId: string): void {
    ...
    const visitForCallsAndStructure = (node: SyntaxNode): void => {
      ...
      // nested-overlap, multi-attribution by design (the generic dispatch
      // below still runs and keeps the enclosing-symbol attribution).
  ...
  private walkCsharpTypePosition(node: SyntaxNode, fromNodeId: string): void {
    ...
    if (node.type === 'identifier') {
      const name = getNodeText(node, this.source);
      if (name && !this.isBuiltinType(name)) {
        this.unresolvedReferences.push({
          fromNodeId,
          referenceName: name,
          referenceKind: 'references',
    ...
    if (node.type === 'qualified_name') {
      ...
      if (last && !this.isBuiltinType(last)) {
        this.unresolvedReferences.push({
          fromNodeId,
          referenceName: last,
          referenceKind: 'references',
...
export function extractFromSource(
  ...
  if (detectedLanguage === 'svelte') {
    ...
    // language (DEFAULT_ROUTED is lua/luau since wave 1 plus the tsjs family
    //   typescript/tsx/javascript/jsx since the wave-2 byte-identical opening,
    //   plus kotlin/scala/dart since wave 3
  ...
  // Framework-specific extraction (routes, middleware, etc.) — reuses the
  // list computed up front for the kernel gate (same filter, same order).
