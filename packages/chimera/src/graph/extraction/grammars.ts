...
import type { Parser, Language as WasmLanguage } from '../web-tree-sitter-types';

// Fork runtime loader (kept from the fork's grammars.ts): the compiled single
// binary and dist layouts can't always resolve a bare 'web-tree-sitter' ESM
// import, so the runtime is acquired through a multi-level cjs require
// fallback (execPath-adjacent → dist-relative node_modules → cwd → bare
// specifier).
...
function isTreeSitterRuntime(value: unknown): value is TreeSitterRuntime {
  const candidate = value as Partial<TreeSitterRuntime> | undefined;
  return typeof candidate?.Parser === 'function' && typeof candidate.Language === 'function';
}

function treeSitterRuntime(value: unknown): TreeSitterRuntime | null {
  if (isTreeSitterRuntime(value)) return value;
  const candidate = value as { default?: unknown } | undefined;
  return isTreeSitterRuntime(candidate?.default) ? candidate.default : null;
}
...
function loadTreeSitterRuntime(): TreeSitterRuntime {
  for (const fallbackPath of [
    ...
    try {
      const fallbackRuntime = treeSitterRuntime(require(fallbackPath));
  ...
  throw new Error('Unable to load web-tree-sitter runtime');
}

const TreeSitter = loadTreeSitterRuntime();
...
/**
 * Fork runtime resolution order (the fork's multi-level loader is kept): a
 * vendored wasm first (revision-matched to the kernel's Cargo.toml pin;
 * provenance in wasm/MANIFEST.md), then the tree-sitter-wasms npm package for
 * non-vendored languages (objc, solidity), then the binary-adjacent
...
function resolveGrammarWasmPath(lang: GrammarLanguage): string {
  ...
  if (VENDORED_WASM_LANGS.has(lang) && fs.existsSync(vendoredPath)) return vendoredPath;

  if (!VENDORED_WASM_LANGS.has(lang)) {
    try {
...
export async function readGrammarWasmBytes(languages: Language[]): Promise<Record<string, Uint8Array>> {
  ...
  for (const lang of toRead) {
    try {
      out[lang] = await fsp.readFile(resolveGrammarWasmPath(lang));
    } catch {
...
export async function loadGrammarsForLanguages(languages: Language[], wasmBytes?: Record<string, Uint8Array>): Promise<void> {
  ...
  for (const lang of toLoad) {
    try {
      ...
      const language = await TreeSitter.Language.load(bytes ?? resolveGrammarWasmPath(lang));
