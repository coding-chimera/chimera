import type { Node as SyntaxNode } from '../../web-tree-sitter-types';
...
export const csharpExtractor: LanguageExtractor = {
  ...
  // directly by the property/field paths, not through returnField). Upstream's
  // own type-ref paths compensate already (extractCsharpReturnType reads
  // 'returns'; extractCsharpTypeRefs checks 'type' ?? 'returns'); this value
  // serves the fork's RAW returnType wire field (returnTypeNode/returnTypeText
  // in tree-sitter.ts), whose generic returnField read has no language gate
  // (K-v2 P0 D6).
  returnField: 'returns',
