...
function rowToFileSemanticRecord(row: FileSemanticRow): FileSemanticRecord {
  return {
    path: row.path,
    contentHash: row.content_hash,
    semantic: {
      schemaVersion: row.schema_version as FileSemanticInfo['schemaVersion'],
      classifierVersion: row.classifier_version as FileSemanticInfo['classifierVersion'],
      role: row.role as FileSemanticInfo['role'],
      confidence: row.confidence as FileSemanticInfo['confidence'],
      source: row.source as FileSemanticInfo['source'],
      reason: row.reason,
      signals: safeJsonParse<string[]>(row.signals_json, []),
    },
    updatedAt: row.updated_at,
  };
}
...
export class QueryBuilder {
  ...
  private stmts: {
    ...
    upsertFileSemantic?: SqliteStatement;
    getFileSemanticByPath?: SqliteStatement;
  ...
  searchNodesDetailed(query: string, options: SearchOptions = {}): SearchNodesDetailedResult {
    ...
    // folds Unicode, which would silently stop matching non-ASCII names. (#1542)
  ...
  findNodesByExactName(names: string[], options: SearchOptions = {}): SearchResult[] {
    ...
    // the corpus and never reach either LIMIT. (#1542)
  ...
  upsertFileSemantic(record: FileSemanticRecord): void {
    ...
    this.stmts.upsertFileSemantic.run({
      path: record.path,
      contentHash: record.contentHash,
      schemaVersion: record.semantic.schemaVersion,
      classifierVersion: record.semantic.classifierVersion,
      role: record.semantic.role,
      confidence: record.semantic.confidence,
      source: record.semantic.source,
      reason: record.semantic.reason,
      signalsJson: JSON.stringify(record.semantic.signals),
  ...
  getFileSemanticByPath(filePath: string): FileSemanticRecord | null {
    if (!this.stmts.getFileSemanticByPath) {
      this.stmts.getFileSemanticByPath = this.db.prepare('SELECT * FROM file_semantics WHERE path = ?');
    }
    const row = this.stmts.getFileSemanticByPath.get(filePath) as FileSemanticRow | undefined;
    return row ? rowToFileSemanticRecord(row) : null;
  }
   ...
   *
   * Deliberately NOT {@link getNodeNamesByFiles}: a bare name set is taken over
   * the WHOLE changed batch, so a name that moves between two files in one
   * commit (or exists in one changed file and is newly added to another)
   * appears on both sides and cancels out of the symmetric difference — even
   * though a definition genuinely appeared or vanished and every reference to
   * that name repo-wide may now bind elsewhere. Keying by file makes each
   * definition its own fact, so the move is seen as one removal plus one
   * addition.
   */
