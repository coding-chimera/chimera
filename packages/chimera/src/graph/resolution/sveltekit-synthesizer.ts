 ...
 * `callback-synthesizer.ts`'s `svelteKitLoadEdges` upstream. FORK STATUS (G-C):
 * that pass is not yet in this fork's callback-synthesizer (later batch); a
...
export function svelteKitLinkEdges(ctx: ResolutionContext): Edge[] {
 ...
 */
export function svelteKitPageComponentEdges(ctx: ResolutionContext): Edge[] {
  const table = svelteKitTable(ctx);
  if (table.byRoot.size === 0) return [];
  const edges: Edge[] = [];
