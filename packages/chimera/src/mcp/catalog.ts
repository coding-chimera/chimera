...
// Some servers advertise an outputSchema whose $ref cannot be resolved, which makes the
// strict tools/list schema reject the whole page. Retry those pages without outputSchema so
// the tools stay discoverable; every other validation error still fails the request.
...
export function convertTool(mcpTool: MCPToolDef, client: Client, timeout?: number): Tool {
  // Spread first, then override type so a server-supplied inputSchema cannot change it.
  ...
  return dynamicTool({
    ...
    execute: async (args: unknown, options) => {
      ...
      // Prefer real content blocks; only synthesize text when the server sent structured output alone.
...
export function fetch<T extends { name: string }>(
  ...
  return Effect.tryPromise({
    ...
    Effect.map((items) => {
      ...
      // Escape both the separator and the escape marker so `server:uri` keys stay unambiguous.
