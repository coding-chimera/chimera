  ...
  const body = input.parentSessionID
    ...
    : variant === "ultra"
      ? isDeepSeekModel(input.model)
        ? "Proactive multi-agent delegation is active for this root session. Exploration is a swarm task by default: for repository investigation, architecture, call-flow, or impact mapping, and any multi-file survey, split the work into independent sub-questions (by file, module, symbol, or evidence source) and fan them out with `chimera_swarm` using up to 16 parallel workers. Treat 16 as the normal operating width, not a reserved ceiling. Do not assume you can complete broad exploration alone; even with only a few known files, delegate the independent sub-items whenever you judge exploration is needed. The parent agent still owns synthesis, conflict resolution, final audit, and verification. Keep work local only when sub-items share files, have ordering constraints, or fan-out adds no material benefit."
