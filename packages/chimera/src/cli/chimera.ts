 ...
 * Chimera CLI
 *
 * Command-line interface for Chimera semantic code intelligence.
 *
 * Usage:
 ...
 *   chimera context <task>     Build context for a task
 *   chimera callers <symbol>   Find what calls a function/method
...
async function loadCodeGraph(): Promise<typeof import('../index')> {
  try {
    ...
    console.error(`\n  Error: ${msg}`);
    console.error('\n  Try reinstalling with: npm install -g @opencode-ai/chimera\n');
    process.exit(1);
  }
}
...
program
  ...
  .action(async (pathArg: string | undefined, options: { force?: boolean }) => {
    ...
    try {
      ...
      success(`Removed Chimera from ${projectPath}`);
    } catch (err) {
...
program
  ...
  .action(async (pathArg: string | undefined, options: { json?: boolean }) => {
    ...
    try {
      ...
      // Surface the active SQLite backend (runtime built-in real SQLite, full
      // WAL + FTS5, no native build).
      const backendLabel = backend === 'bun-sqlite'
        ? chalk.green(`bun:sqlite ${getGlyphs().dash} Bun built-in (full WAL)`)
        : chalk.green(`node:sqlite ${getGlyphs().dash} Node built-in (full WAL)`);
      console.log(`  Backend:   ${backendLabel}`);
      // Effective journal mode: 'wal' means concurrent reads never block on a
      // writer; anything else means they can ("database is locked"). The active
      // backend supports WAL, so a non-wal mode means the filesystem can't
 ...
 *
 * Usage:
 *   git diff --name-only | chimera affected --stdin
 *   chimera affected src/lib/components/Editor.svelte src/routes/+page.svelte
 */
program
