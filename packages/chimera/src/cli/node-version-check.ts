 ...
 * reliably crashes Chimera with `Fatal process out of memory: Zone`
...
/**
 * Build the bordered banner shown when Chimera detects an
...
export function buildNode25BlockBanner(nodeVersion: string): string {
  ...
  return [
    sep,
    `[Chimera] Unsupported Node.js version: ${nodeVersion}`,
    sep,
 ...
 * package.json. Below this, Chimera relies on language features / native APIs
...
/**
 * Build the bordered banner shown when Chimera detects a Node.js major below
...
export function buildNodeTooOldBanner(nodeVersion: string): string {
  ...
  return [
    ...
    `[Chimera] Unsupported Node.js version: ${nodeVersion}`,
    sep,
    `Chimera requires Node.js ${MIN_NODE_MAJOR} or newer. Older versions lack`,
    'language features and native APIs Chimera depends on, and are not',
    'tested or supported.',
    '',
    'Fix: install Node.js 22 LTS:',
    '  nvm install 22 && nvm use 22                          # nvm',
