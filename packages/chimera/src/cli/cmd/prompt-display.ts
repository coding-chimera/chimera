/**
 * Display-width helpers for the TUI prompt (upstream packages/tui/src/prompt/display.ts,
 * absorbed with fix(tui) bba76009a8): textarea offsets are display columns, not
 * string indices — wide characters (CJK, emoji) occupy two columns, so slicing a
 * prompt string at an extmark offset must go through grapheme-aware width
 * accounting or pasted placeholders corrupt the surrounding text.
 */
