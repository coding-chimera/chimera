...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall Chimera and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Chimera")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g chimera",
      pnpm: "pnpm uninstall -g chimera",
      bun: "bun remove -g chimera",
      yarn: "yarn global remove chimera",
      brew: "brew uninstall chimera",
      choco: "choco uninstall chimera",
      scoop: "scoop uninstall chimera",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "chimera", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".chimera") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Chimera!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      content.includes("# chimera") ||
      content.includes("# opencode") ||
      content.includes(".chimera/bin") ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# chimera" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".chimera/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        (trimmed.includes(".chimera/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".chimera") || trimmed.includes(".opencode")))
    ) {
      continue
    }
