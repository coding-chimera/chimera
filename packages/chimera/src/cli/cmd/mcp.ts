...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: chimera mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in chimera.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .chimera/ subdirectory too)
  const candidates = [path.join(baseDir, "chimera.json"), path.join(baseDir, "chimera.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ConfigPaths.APP_CONFIG_DIR, "chimera.json"),
      path.join(baseDir, ConfigPaths.APP_CONFIG_DIR, "chimera.jsonc"),
    )
  }
  ...
  // Default to chimera.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., chimera x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
...
export const McpDebugCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (entry?.tokens) {
        prompts.log.info(`  Access token: ${entry.tokens.accessToken.slice(0, 4)}***${entry.tokens.accessToken.slice(-4)}`)
