...
import { ConfigDelegation } from "./delegation"
...
export const Info = Schema.Struct({
  ...
  delegation: Schema.optional(ConfigDelegation.Info).annotate({
    description: "Subagent model delegation: named model profiles and role routes",
  }),
  tools: Schema.optional(Schema.Record(Schema.String, Schema.Boolean)),
  attachment: Schema.optional(ConfigAttachment.Info).annotate({
    description: "Attachment handling configuration",
  }),
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://coding-chimera.github.io/chimera/schemas/config.json" }, null, 2))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            log.debug("fetching remote config", { url: `${url}/.well-known/chimera` })
            const response = yield* Effect.promise(() => fetch(`${url}/.well-known/chimera`))
            if (!response.ok) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(ConfigPaths.APP_CONFIG_DIR) || dir === Flag.OPENCODE_CONFIG_DIR) {
          ...
          // Auto-discovered plugins under `.chimera/plugin(s)` are already local files, so ConfigPlugin.load
