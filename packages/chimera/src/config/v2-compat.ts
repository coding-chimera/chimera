...
const decodeLspEntry = Schema.decodeUnknownOption(ConfigLSP.Entry, decodeOptions)
const builtinServers = new Set<string>(ConfigLSP.builtinServerIds)
...
export function lower(input: unknown, source = "configuration"): Result {
  ...
  // Fork adaptation: Chimera's `ConfigParse.effectSchema` rejects unknown top-level
  // keys instead of dropping them like upstream's excess-property-ignoring decoder.
  // Delete the consumed V2 source keys here so the lowered projection stays decodable.
...
function normalizeSettings(input: Record<string, unknown>, result: Record<string, unknown>, diagnostics: Diagnostic[]) {
  ...
  if (Object.hasOwn(input, "media")) {
    const value = decodeValue(ConfigAttachment.Info, input.media, ["media"], diagnostics)
