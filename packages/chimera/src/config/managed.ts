...
const MANAGED_PLIST_DOMAIN = "ai.chimera.managed"

// Keys injected by macOS/MDM into the managed plist that are not Chimera config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/chimera"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "chimera")
    default:
      return "/etc/chimera"
  }
}
