#!/usr/bin/env node
/**
 * Diff two Chimera graph indexes of the SAME tree — typically a live,
 * incrementally-synced `.chimera/codegraph.db` against a clean full rebuild
 ...
 *   cp .chimera/codegraph.db /tmp/live.db
 *   chimera graph index --force .
 *   node packages/chimera/scripts/agent-eval/diff-index-drift.mjs /tmp/live.db .chimera/codegraph.db
 *
 *   # or rebuild into a scratch data root, leaving the live index untouched
 *   CHIMERA_DATA_DIR=/tmp/rebuild chimera graph init .
 *   CHIMERA_DATA_DIR=/tmp/rebuild chimera graph index .
 *   node packages/chimera/scripts/agent-eval/diff-index-drift.mjs .chimera/codegraph.db /tmp/rebuild/codegraph.db
 *
 ...
 * upstream codegraph repo the raw counts differed by +0.7% while 4.3% of
 * distinct edges were actually wrong.
 */
...
// former: on the upstream codegraph repo every check below was 0 on BOTH
// indexes.
console.log('');
