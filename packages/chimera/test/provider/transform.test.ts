    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".chimera/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    expect(result[0].content[0].providerOptions?.chimera?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.chimera?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.chimera?.itemId).toBe("msg_opencode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.chimera?.itemId).toBe("msg_opencode_part")
...
describe("ProviderTransform.variants", () => {
  ...
  describe("@ai-sdk/openai-compatible", () => {
    ...
    test("north-mini-code-1-0 returns only none and high", () => {
      ...
      const result = ProviderTransform.variants(model)
      expect(Object.keys(result)).toEqual(["none", "high", "ultra"])
  ...
  describe("@ai-sdk/amazon-bedrock/mantle", () => {
    test("gpt-5.5 returns OpenAI-style reasoning variants", () => {
      ...
      const result = ProviderTransform.variants(model)
      expect(Object.keys(result)).toEqual(["none", "low", "medium", "high", "xhigh", "ultra"])
      expect(result.medium).toEqual({
