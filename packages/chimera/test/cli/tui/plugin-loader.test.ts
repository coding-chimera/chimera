...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".chimera", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".chimera", "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".chimera", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".chimera", "themes", themeFile)
...
`,
  ...
  try {
    await TuiPluginRuntime.init({ api: mkApi(), config: mkConfig(), waitForDependencies })
    await TuiPluginRuntime.dispose()
