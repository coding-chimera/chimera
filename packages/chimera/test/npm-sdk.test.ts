 ...
 * `require("@opencode-ai/chimera")` on their OWN Node by re-exporting the
 * compiled library that ships inside the per-platform optionalDependency
 * (@opencode-ai/chimera-<target>/lib/dist/index.js).
 *
...
// SDK's `require.resolve('@opencode-ai/chimera-<target>/...')` walks to it.
function makeConsumer(): { root: string; mainPkg: string } {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: '@opencode-ai/chimera', version: VERSION, main: 'npm-sdk.js' }) + '\n'
  );
  return { root, mainPkg };
}
...
function installPlatformPackage(root: string, sentinel: string): void {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: `@opencode-ai/chimera-${target}`, version: VERSION }) + '\n'
  );
}
...
describe('npm-sdk programmatic entry', () => {
  ...
  it('throws an actionable error when no bundle is installed or cached', () => {
    ...
    expect(r.stderr).toContain(`@opencode-ai/chimera-${target}`);
