...
describe("instance HttpApi", () => {
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-chimera-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-chimera-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-chimera-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-chimera-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-chimera-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const current = await app().request("/project/current", { headers: { "x-chimera-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const list = await app().request("/project", { headers: { "x-chimera-directory": tmp.path } })
