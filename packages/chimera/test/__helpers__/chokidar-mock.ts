...
class MockChokidarWatcher extends EventEmitter {
  ...
  close(): Promise<void> {
    ...
    for (const root of this.roots) instancesByRoot.delete(root);
...
export const chokidarMockModule = {
  default: {
    watch: (root: string | string[], _opts?: unknown) => {
      const roots = Array.isArray(root) ? root : [root];
      const inst = new MockChokidarWatcher(roots);
      for (const item of roots) instancesByRoot.set(item, inst);
      return inst;
    },
  },
};
