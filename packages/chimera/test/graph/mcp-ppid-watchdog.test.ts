 ...
 * propagate the death to its `chimera serve --mcp` child. The child gets
...
const bunRuntimeCannotPipeReadableStdio = typeof (process.versions as Record<string, string | undefined>).bun === 'string';
...
// Bun's Node-compatible child_process.spawn currently throws
// `TODO: stream.Readable stdio @ 0` for the wrapper shape this regression
// needs: a sibling process' stdout passed as the MCP child's stdin. Keep the
// POSIX/Node coverage and skip only the Bun test runner environment.
describe.skipIf(process.platform === 'win32' || bunRuntimeCannotPipeReadableStdio)('MCP PPID watchdog (#277)', () => {
