 ...
 *
 * Parsing, validation, and mtime-caching of the opt-in patterns that
 * re-include gitignored directories for embedded-repo discovery. The
 * behavioral end (extraction honoring the patterns) lives in
 * extraction.test.ts; these are the loader unit tests.
 *
 * Invariant under test: every failure mode degrades to the zero-config
 * default (empty patterns → `.gitignore` fully respected), never a throw.
 */
import { describe, it, expect, beforeEach, afterEach } from './vitest';
import * as fs from 'node:fs';
import * as path from 'node:path';
import * as os from 'node:os';
import { loadIncludeIgnoredPatterns, clearProjectConfigCache, addIncludeIgnoredPatterns } from '../../src/graph/config';
