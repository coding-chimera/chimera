/**
 * ParseWorkerPool — the worker pool that parses files across cores during a
 * full `graph index` (upstream #1015). These tests drive the pool's queue /
 * growth / recycle / crash-recovery / timeout / teardown logic with INJECTED
 * fake workers, so they exercise the real scheduling code without spawning
 * threads or needing a built dist.
 *
...
describe('ParseWorkerPool', () => {
  ...
  it('recycles a worker after recycleInterval parses', async () => {
    ...
    // 3 parses on the first worker → recycle (terminate + respawn); the 4th
    // runs on the fresh worker.
  ...
  it('rejects a parse whose worker crashes (retry-pass-recognisable message) and keeps serving', async () => {
    ...
    // The message must contain "Worker exited" so the orchestrator's retry
    // pass re-attempts it (that's the filter it uses).
  ...
  it('accepts a result that arrives after the base timeout instead of killing the worker (#1231 false-timeout fix)', async () => {
    ...
    // delivered after the base timer fired. Old behaviour killed the worker
    // and rejected; now the late result is accepted and the worker keeps
    // serving.
