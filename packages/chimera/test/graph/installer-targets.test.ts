 ...
 *
 * For agent-config destinations we redirect HOME to a tmpdir via env
 * vars plus `os.homedir` spying, and CWD via `process.chdir` — same
 * pattern as the legacy `installer.test.ts`. No real `~/.claude/`
 * etc. ever touched.
 */
...
import { ALL_TARGETS, getTarget, resolveTargetFlag } from '../../src/graph/installer/targets/registry';
import { uninstallTargets } from '../../src/graph/installer';
import { upsertTomlTable, removeTomlTable, buildTomlTable } from '../../src/graph/installer/targets/toml';
import { cleanupLegacyHooks } from '../../src/graph/installer/targets/claude';

function mkTmpDir(label: string): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), `cg-targets-${label}-`));
}

// Redirect both env vars and `os.homedir()`: Bun keeps returning the
// process' original home after env changes, while the installer target
// modules resolve paths by calling `os.homedir()` at runtime.
...
describe('Installer targets — partial-state idempotency', () => {
  ...
  it('antigravity: install migrates a legacy codegraph entry to the unified path when marker appears', () => {
    ...
    // the unified file). Re-running chimera install should land
  ...
  it('claude: uninstall strips codegraph from ./.mcp.json and a legacy ./.claude.json', () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ mcpServers: { codegraph: { command: 'chimera' } } }, null, 2),
    );
    fs.writeFileSync(
      path.join(tmpCwd, '.claude.json'),
      JSON.stringify({ mcpServers: { codegraph: { command: 'chimera' }, other: { command: 'x' } } }, null, 2),
    );
  ...
  it('claude: uninstall strips stale hooks written in the npx form (local)', () => {
    ...
    const file = seedSettings('local', {
      hooks: {
        PostToolUse: [
          { matcher: 'Edit|Write', hooks: [{ type: 'command', command: 'npx @opencode-ai/chimera mark-dirty', async: true }] },
        ],
        Stop: [
          { hooks: [{ type: 'command', command: 'npx @opencode-ai/chimera sync-if-dirty' }] },
        ],
      },
    });
...
describe('Installer — uninstallTargets sweep (chimera uninstall)', () => {
  let tmpHome: string;
  let tmpCwd: string;
  let origCwd: string;
  let homeRestore: { restore: () => void };
