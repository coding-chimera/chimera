 ...
 *
 * Chimera graph data resolves from current `.chimera/` or compatible legacy `.codegraph/` roots.
...
describe('detectWorktreeIndexMismatch (issue #155)', () => {
  let mainRepo: string;   // main checkout — owns the Chimera graph index
  ...
  it('warning names both trees and the fix', () => {
    ...
    expect(msg).toContain('chimera graph init');
    expect(msg).toContain('chimera graph index');
  });
});
