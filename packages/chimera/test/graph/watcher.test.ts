...
import { chokidarMockModule, triggerFileEvent } from './__helpers__/chokidar-mock';

// Bun does not hoist mock.module ahead of static imports, so register the
// chokidar mock before dynamically importing modules that depend on chokidar.
vi.mock('chokidar', () => chokidarMockModule);

const { FileWatcher, LockUnavailableError } = await import('../../src/graph/sync/watcher');
...
describe('FileWatcher', () => {
  ...
  describe('filtering', () => {
    ...
    it('should include non-source files when requested', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 1, durationMs: 10 });
      const onBatch = vi.fn().mockResolvedValue({ filesChanged: 1, durationMs: 10 });
      const watcher = new FileWatcher(testDir, syncFn, {
        debounceMs: 200,
      ...
      await waitFor(() => syncFn.mock.calls.length > 0);
      expect(onBatch.mock.calls[0]![0].files).toEqual(['src/readme.md']);
    ...
    it('should ignore .git changes by default', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
      const watcher = new FileWatcher(testDir, syncFn, { debounceMs: 200 });

      watcher.start();
      await watcher.waitUntilReady();
      triggerFileEvent(testDir, 'change', '.git/HEAD');
    ...
    it('should ignore nested .git directories from embedded repos', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
      const watcher = new FileWatcher(testDir, syncFn, {
        debounceMs: 200,
        includeNonSource: true,
      ...
      watcher.start();
      await watcher.waitUntilReady();
      triggerFileEvent(testDir, 'add', 'codex/.git/fsmonitor--daemon.ipc');
    ...
    it('should capture git HEAD and current branch ref changes when requested', async () => {
      git(testDir, 'init', '-q');
      const branch = git(testDir, 'symbolic-ref', '--short', 'HEAD');
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 0 });
      const onBatch = vi.fn().mockResolvedValue({ filesChanged: 0, durationMs: 1 });
      const watcher = new FileWatcher(testDir, syncFn, {
        debounceMs: 100,
        autoSync: false,
        watchGitHead: true,
      ...
      await watcher.waitUntilReady();
      triggerFileEvent(testDir, 'change', '.git/HEAD');
      triggerFileEvent(testDir, 'change', `.git/refs/heads/${branch}`);

      await waitFor(() => onBatch.mock.calls.length > 0);
      expect(syncFn).not.toHaveBeenCalled();
      expect(onBatch.mock.calls[0]![0].source).toBe('git');
      expect(onBatch.mock.calls[0]![0].files).toEqual([
        '.git/HEAD',
        `.git/refs/heads/${branch}`,
      ]);
      expect(onBatch.mock.calls[0]![0].events.map((event: { source: string }) => event.source)).toEqual(['git', 'git']);
  ...
  describe('pending file tracking (#403)', () => {
    ...
    it('should retain pending files and retry when syncFn throws LockUnavailableError (#449)', async () => {
      ...
      expect(onSyncComplete.mock.calls[0]![0]).toEqual({ filesChanged: 1, durationMs: 10 });
      expect(onSyncComplete.mock.calls[0]![1]!.files).toEqual(['src/locked.ts']);
      expect(onSyncError).not.toHaveBeenCalled();
  ...
  describe('callbacks', () => {
    it('should call onSyncComplete after successful sync', async () => {
      ...
      expect(onSyncComplete.mock.calls[0]![0]).toEqual({ filesChanged: 2, durationMs: 50 });
      expect(onSyncComplete.mock.calls[0]![1]!.files).toEqual(['src/test.ts']);

      watcher.stop();
    });

    it('should expose debounced batches without auto-syncing', async () => {
      const syncFn = vi.fn().mockResolvedValue({ filesChanged: 1, durationMs: 10 });
      let sawPendingDuringBatch = false;
      const onBatch = vi.fn(async (_batch, api) => {
        sawPendingDuringBatch = api.getPendingFiles().some((p: { path: string }) => p.path === 'src/manual.ts');
      ...
      watcher.start();
      await watcher.waitUntilReady();
      triggerFileEvent(testDir, 'add', 'src/manual.ts');

      await waitFor(() => onBatch.mock.calls.length > 0);
      expect(syncFn).not.toHaveBeenCalled();
      expect(onBatch.mock.calls[0]![0].files).toEqual(['src/manual.ts']);
      expect(onBatch.mock.calls[0]![0].events[0].event).toBe('add');
      expect(sawPendingDuringBatch).toBe(true);

      await waitFor(() => onSyncComplete.mock.calls.length > 0);
      expect(onSyncComplete.mock.calls[0]![0]).toEqual({ filesChanged: 1, durationMs: 5 });
      await waitFor(() => watcher.getPendingFiles().length === 0);
  ...
  describe('CodeGraph integration', () => {
    ...
    it('should allow manual batch handling through CodeGraph.watch', async () => {
      ...
      const initialStats = cg.getStats();
      const onBatch = vi.fn(async (batch, api) => api.syncFiles(batch.files));

      cg.watch({ debounceMs: 200, autoSync: false, onBatch });
      await cg.waitUntilWatcherReady();

      fs.writeFileSync(
        path.join(testDir, 'src', 'manual.ts'),
        'export function manual() { return 7; }'
      );
      triggerFileEvent(testDir, 'add', 'src/manual.ts');

      await waitFor(() => onBatch.mock.calls.length > 0);
      expect(onBatch.mock.calls[0]![0].files).toEqual(['src/manual.ts']);
      expect(onBatch.mock.calls[0]![1].snapshot().revision).toBeTruthy();
