 ...
 * upstream codegraph's own repo. Two mechanisms, both exercised here:
 *
 ...
 * bun-test shim imports, `.chimera` data root resolved via getGraphDataRootInfo
 * (never hard-coded), initSync without a config argument, a rebuild that is
 * clear() + indexAll on the live handle (the fork has no CodeGraph.recreate,
 * and initSync refuses an already-initialized root) — exactly what
 * `graph index --force` does and a genuine rebuild (every row is deleted
 * first, so the store cannot no-op on identical hashes) — and function-source
 * edge counting where the fork's value-ref co-emission doubles raw rows.
 *
 * ---
 *
 ...
 * (`src/graph/index.ts`, guarding `resurrectStaleResolutionEdges`). The
 * convergence cases below are the only coverage that half has, so the check is
 * the suite's own regression test:
 *
 *     CODEGRAPH_NO_REBIND=1 bun test test/graph/sync-rebuild-convergence.test.ts
 *
 * must report failures, and an unset run must be green. If you change a case
 * here, re-run both. Upstream, a version of this suite passed under the kill
 * switch because `rebuildEdgeSet` was not rebuilding anything — keep the
 * clear() in the rebuild path.
 */
...
import type { SqliteDatabase } from '../../src/graph/db/sqlite-adapter';
...
describe('Incremental sync converges to a full rebuild (CG-33)', () => {
  ...
  const dbPath = (): string => getGraphDataRootInfo(testDir).databasePath;
   ...
   * (generateNodeId), so for an identical tree they are identical across a sync
   * and a rebuild — which is what makes the two sets directly comparable.
   */
   ...
   * set — the ground truth a user gets from `chimera graph index --force`.
   *
   * It must DELETE every row first (clear()): calling indexAll on the live
   * handle instead is not a rebuild at all — every file hashes identical, so
   * the store writes nothing (`nodesCreated: 0`), no reference is re-created,
   * and every existing edge survives untouched. The comparison then reads the
   * synced index against ITSELF and can never fail, which is exactly how the
   * upstream version of this suite once passed with `CODEGRAPH_NO_REBIND=1`
   * (CG-35).
   */
  const rebuildEdgeSet = async (): Promise<Set<string>> => {
    // Same-handle rebuild: clear() deletes every row (files included), so the
    // indexAll that follows re-extracts the whole tree from nothing — the fork
    // has no CodeGraph.recreate, and initSync refuses an already-initialized
    // root. This mirrors `chimera graph index --force` (cg.clear() + indexAll).
    cg.clear();
    await cg.indexAll();
    return edgeSet();
  };
  ...
  it('rolls back edge deletion when requeueing its reference is interrupted', async () => {
    ...
    try {
      ...
      threw = /forced rebind interruption/.test(String(error));
    }
   ...
   * changed file already defines it — which is precisely the shape a commit that
  ...
  it('declines a name over the per-name ceiling instead of rebinding an arbitrary subset', async () => {
    ...
    // Restrict to function-source edges: the fork's extractor co-emits a
    // statement-source `calls` edge alongside the function-source one for the
    // same call site (value-ref co-emission), which would double raw counts.
          ...
          db
            .prepare(
              `SELECT t.file_path AS file FROM edges e
               ...
               WHERE t.name = ? AND e.kind = 'calls' AND s.kind = 'function'`
            )
    ...
    // This is knowingly divergent from a rebuild; the ceiling trades that
    // residual drift against manufacturing wrong edges on generic names.
