/**
 * Vue Router (port of upstream vue-router.test.ts, G-C): routes read out of
 ...
 * template.
 *
 * FORK NOTE: upstream's final case ("lands on the Screens tab") is NOT ported —
 * this fork has no `ui-server/api/screens` surface. Everything below it is the
 * upstream file verbatim, adapted to the fork's test conventions (vitest shim,
 * `CodeGraph.init` + `indexAll`).
 */
...
import { parseVueRoutes, vueNavVerb, routeNameInExpression } from '../../src/graph/resolution/frameworks/vue-router';
