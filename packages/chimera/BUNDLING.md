...
Chimera can ship a **vendored Node runtime** alongside the app. Because Node 22.5+
...
chimera-<target>/
  ...
  bin/
    chimera | chimera.cmd       # launcher → runs the bundled Node with the app
```
...
```bash
scripts/build-bundle.sh linux-x64            # -> release/chimera-linux-x64.tar.gz
scripts/build-bundle.sh win32-x64            # -> release/chimera-win32-x64.zip
```
...
1. **`curl | sh`** ([`install.sh`](install.sh)) — no Node required; ideal for a
   ...
   Releases, symlinks `chimera` onto PATH. Re-run to upgrade; `--uninstall` to
   remove.
2. **npm** ([`scripts/npm-shim.js`](scripts/npm-shim.js)) — preserves
   `npm i -g @opencode-ai/chimera`. The main package is a tiny shim; the
   bundles ship as per-platform `optionalDependencies`
   (`@opencode-ai/chimera-<target>` with `os`/`cpu`), so npm installs only the
...
- Retire the now-vestigial Node-version gate in `src/cli/chimera.ts` — the
