...
- Change-tool results carry a reactive audit hint: successful `edit`/`write`/`apply_patch` outputs append `Propagation audit recommended: run chimera_audit_recent before treating this change as complete (skip only when the edit is trivial or intentionally scoped).` to the result tail. The reminder is event-driven (B4), costs nothing when those tools are idle, and is not duplicated in prompt text.
