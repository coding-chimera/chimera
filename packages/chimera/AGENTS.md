...
- For local tarball/user-install validation, build the default no-WebUI variant with `OPENCODE_CHANNEL=latest bun run build --single --skip-install`. This creates `dist/npm-tarballs/coding-chimera-chimera-no-webui-<version>.tgz` and the current-platform tarball such as `coding-chimera-chimera-darwin-arm64-no-webui-<version>.tgz`.
