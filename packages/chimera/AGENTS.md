...
- For local tarball/user-install validation, build the default no-WebUI variant with `bun run build --single --skip-install`. This creates `dist/npm-tarballs/chimera-no-webui-<version>.tgz` and the current-platform tarball such as `chimera-darwin-arm64-no-webui-<version>.tgz`.
