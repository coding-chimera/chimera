...
- Change-tool results carry an inline propagation summary: successful `edit`/`write`/`apply_patch` outputs append a bounded probe line from `src/chimera/propagation-probe.ts` (`Propagation check: <N> dependent file(s) may be affected: <top 3 paths>.` or the no-dependents wording) as pure informational context, with no action invitation and no `chimera_audit_recent` reference (B4). When the session has a predesign declaration, the probe additionally emits `Scope check:` lines reconciling edit targets and a bounded transitive walk over dependents (depth-capped, node-capped, cycle-safe via a visited set, continuing only through pass-through-like small files ≤40 lines) against the declared file scope; out-of-scope entries are ranked deepest-hop-first and annotated `via <intermediate>, <N> hops`, because multi-hop consumers are invisible to grep on the changed symbol and often hardcode the stale contract (TB6 bench: 3-hop and cascade fixtures). When the graph is unavailable or the probe exceeds its 500ms budget, it degrades silently and no line is appended.
