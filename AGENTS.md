...
- Environment-specific extra requirements for this checkout are kept in the root `env.md` (local-only, never committed, excluded via `.git/info/exclude`); read it before committing when present. When `env.md` is absent, ask the user once whether any checkout-local requirements should be recorded there; if the user declines, create an empty `env.md` placeholder so the question is not asked again.
- Subagent scheduling: never run subagents on `kimi-k3` (it is the root-session model in this checkout; keep children on other routes). Builders follow size-tiered effort: large models (L/XL) take a low reasoning `variant`, small flash-class models take `variant: high`, and `model` + `variant` are always passed explicitly so dispatch never auto-fills. Scouting/exploration declares `workload: scout` and lets the scheduler pick; fan out with swarm. qwen3.8-flash is excluded from `scout` via `delegation.scheduling.archetypes.scout.excludeModels` in the global config (repo-understanding defect; scout conclusions are not reliably re-verified downstream) and remains eligible for `builder`; never declare `workload: scout` with qwen3.8-flash — dispatch fails with an exclusion error.
