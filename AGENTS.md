...
- For every local tarball, user-install, and release-matrix validation build from `packages/chimera`, embed the NewWeb/WebUI assets and set a stable latest channel: `OPENCODE_CHANNEL=latest bun run build --single --skip-install --with-webui`. This creates `dist/npm-tarballs/chimera-with-webui-<version>.tgz` and the current-platform tarball such as `chimera-darwin-arm64-with-webui-<version>.tgz`.
...
- User-facing and release-matrix global install validation must use npm, not Bun. Install the with-WebUI variant with npm, e.g. `npm install -g ./dist/npm-tarballs/chimera-darwin-arm64-with-webui-<version>.tgz ./dist/npm-tarballs/chimera-with-webui-<version>.tgz`.
